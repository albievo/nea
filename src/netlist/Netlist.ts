...
export class Netlist {
  ...
  private validateNewConnection(connection: Connection): string | null {
    const existing = this.connectionsById.get(connection.id);
    if (existing) return `connection with id ${connection.id} already exists`;

    const fromNode = this.nodesById.get(connection.from.nodeId);
    if (!fromNode) return `node with id ${connection.from.nodeId} does not exist`;

    const fromPinExists = connection.from.outputIdx < fromNode.getOutputNum();
    if (!fromPinExists) return `node with id ${connection.from.nodeId} only has ${fromNode.getOutputNum()} outputs, but the connection is reffering to output of index ${connection.from.outputIdx}`;

    const toNode = this.nodesById.get(connection.to.nodeId);
    if (!toNode) return `node with id ${connection.to.nodeId} does not exist`;

    const toPinExists = connection.to.inputIdx < toNode.getInputNum();
    if (!toPinExists) return `node with id ${connection.to.nodeId} only has ${fromNode.getInputNum()} inputs, but the connection is reffering to input of index ${connection.to.inputIdx}`;

    return null;
  }

  private addConnectionToOutputIndex(connection: Connection): void {
    const existing = this.outputIndex.get(connection.from);
    ...
    this.outputIndex.set(connection.from, [connection]);
  }
